...
* Many of the builtin features use multithreading to speedup processing. For example the CSVFeed parses CSV files in parallel so directories with thousands of CSV files can be parsed in seconds.
...
To give some idea, a simple back test can easily run over 5.000.000 candlesticks per second on modern hardware.
