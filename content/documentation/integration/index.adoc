...
You can combine several of these 3rd parties together in a single run, for example the data feeds from IEX Cloud and the broker functionality from Alpaca.

One of the design principles is that roboquant tries to avoid exposing 3rd party API's directly and typically wraps them in roboquant equivalent types. This makes it easier to switch between different providers and also reduces the learning curve.
