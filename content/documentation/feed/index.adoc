...
== Event
An event contains all the data that happens at a certain point in time and contains the following two attributes

. The `time` when the information within the event became available.
...
* RandomWalk that (as the name already suggests) generates random price actions
* Many third party data providers that make their data available through an API
