...
A feed wraps data in an event and put that event in a EventChannel. An event contains all the data that happens at a certain point in time. The order in which a feed sends out events has to be ordered by the included timestamp.
