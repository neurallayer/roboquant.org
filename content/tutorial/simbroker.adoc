...
TIP: if you are trading specific markets, like FOREX, you most likely want to use custom configuration since the default values like those selected for spreads are not representative for those markets.
...
== Order Handling
When an order is placed at the SimBroker, it only gets processed once there is a known price for the underlying asset. Till an event happens that has that price action available, the order will remain in its `INITIAL` state.
...
Any open orders will be processed based on FIFO (first-in-first-out) basis, so older open orders are processed first. Exception to this rule are modify orders that are always processed before create order types. The most common modify orders are a `CancelOrder` and an `UpdateOrder`.


== Custom Configuration
Custom models are a way to further customize the behavior of `SimBroker`. It is important to notice that if you don't take these type of cost into account, your profitable strategy in back-test might turn out not to be that profitable in live trading.
...
. PricingEngine: calculate the price to use for a trade. It can be used to simulate the following type of cost:
    - Spread (difference between buy and sell price)
    - Slippage (large orders will require multiple trades against different prices)
    - Exchange rates

. FeeModels: the transaction related fees and commissions. Cost to take into consideration:
    . Broker commission fees
    . Borrowing cost when shorting
    . Storage & custodial fees
    . Exchange fees and/or rebates

. AccountModel: model the the different types of accounts, like a Cash or Margin account.

Below are the default included models with their configuration parameters:
...
== Limitations
Like any simulation, SimBroker also has limitations compared to a live trading. It is important to realize this and take appropriate actions where possible:
...
Out of the box _roboquant_ comes with two models that cather for the most common account types:
...
`CashAccount` only base the buying power on the available cash, and no margin is calculated and no leverage is used. So cash and buying power are equal, however cash can contain multiple currencies while the buying power is always converted to the base currency of the account.
