...
== Order Handling
When an order is placed at the SimBroker, it only gets processed once there is a known price for the underlying asset. Till an event happens that has that price info available, the order will remain in the `INITIAL` state.
...
- Cost models: the transaction related costs like slippage and fees/commissions. It is important to notice that if you don't take these type of cost into account, your profitable strategy in back-test might turn out not to be that profitable in live trading. Costs to take into consideration:
...
== Limitations
Like any simulation, SimBroker also has limitations compared to real life trading. It is important to realize this and take appropriate actions where possible:
...
Out of the box _roboquant_ comes with several models that cater for common account types:
...
`CashBuyingPowerModel` base the buying power only on the available cash, and no margin is calculated and no leverage is used. So cash and buying power are equal, however cash can contain multiple currencies while the buying power is always converted to the base currency of the account.
