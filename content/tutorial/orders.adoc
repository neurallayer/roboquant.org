...
TIP: During back testing make sure to only use order types that are also supported by the broker you plan to use for live trading. Also be aware that the way advanced order types are executed might differ slightly between brokers.
...
TIP: When creating a new order in a Policy, always validate if you still have an open order for the same asset and take the appropriate action.
