...
TIP: Always make sure to only use during back testing order types that are also supported by the broker you want to use for live trading. Also be aware that the way these advanced order types are exactly executed might differ between brokers.
...
The default TIF for orders in _roboquant_, if none is explicitly specified, is GTC (Good Till Cancelled). All TIF policies only become active once an `Order` has been `ACCEPTED` (so it's not anymore in the `INITIAL` state).
