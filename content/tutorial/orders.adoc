...
The default TIF for orders in _roboquant_ used if none is explicitly specified, is GTC (Good Till Cancelled). All TIF policies are only becoming active once an `Order` has been `ACCEPTED` (so not anymore in its `INITIAL` state)
