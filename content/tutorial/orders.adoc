...
NOTE: there is no explicit notion of BUY or SELL orders in _roboquant_. A BUY order is just any order with a positive size and a SELL order has a negative size. But other than that, they are equal and created the same way.
...
The default TIF for orders in _roboquant_ used if none is explicitly specified, is GTC (Good Till Cancelled).
