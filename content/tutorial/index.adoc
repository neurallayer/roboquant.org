...
But there is a good reason it is done this way: it is now almost impossible to "look into the future" with your strategy since the broker will always first observe a new event. So the signals and orders you create based on event at time t will only be processed by the broker based on an event at time t+1.
