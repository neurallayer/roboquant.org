...
But there it a good reason it is done this way. It is to avoid a look-ahead bias since the broker will always observe a new event first. So any signals and orders you create based on event at time~t~, will only be processed by the broker based on an event at time~t+1~.
