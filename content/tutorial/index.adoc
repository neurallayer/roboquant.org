...
== Why use roboquant
Developing and running algo-trading software is not an easy task at all. You already need additional skills and on top of that there is a lot of complexity involved that isn't perhaps clear from the beginning. So it makes sense so separate the logic that is common to all automated trading strategies from the logic that is specific to your strategy.
