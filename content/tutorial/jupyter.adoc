...
This will automatically load the _roboquant_ library (it will actually load three _roboquant_ modules: core, extra and crypt) and import most packages from the core module. If you run `Welcome()` in a notebook cell, you'll see the version of _roboquant_ you are using together with other information about the environment you are in.
