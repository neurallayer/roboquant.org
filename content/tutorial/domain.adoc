...
== Time
In order to make it easier to trade in multiple regions, _roboquant_ introduces a number of time related domain classes: <<Timeline>>, <<Timeframe>>, <<TimePeriod>>.
...
The `TimePeriod` class in _roboquant_ unifies this into a single implementation, and make it easier to use them in combination with `Timeframe`, `Instant` and `ZonedDateTime` instances.
