...
=== Assets, Exchanges and TradingCalendars
An asset is linked to a single exchange and that exchange has a time-zone associated with it. The exchange also has a `TradingCalendar` associated with it, that determines what are the valid trading hours for that particular exchange.
