...
== Configuration
When you want to integrate with a 3rd party, you most likely require credentials to gain access to their APIs. Often this is in the form of an API key and secret. You can include these credentials directly in your source code, however _roboquant_ offers better and more secure alternatives.
...
=== Alpaca
The integration with Alpaca covers their broker API as well as their historic and live market data. In order to use this, you'll need at least register and get a paper-trading account with an API-key.
...
== ECB
The ECB (European Central Bank) publishes since the start of the Euro daily the historic exchange rates with almost all other fiat currencies in the world. Roboquant can automatically download these exchange rates from their website and use them for currency conversions during trading.
