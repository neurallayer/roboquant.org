...
When creating a new notebook instance from the SageMaker menu, open the `additional configuration` option and select `create new lifecycle configuration` from the lifecycle configuration dropdown. Then add the following snippet to the `Create notebook` tab.
...
# This script installs a JDK and Kotlin kernel.
...
This has been tested with the platform identifier set to "Amazon Linux 2/Jupyter Lab 1" and "Give users root access to the notebook" enabled (default).
