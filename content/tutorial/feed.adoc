...
== What is a feed
A feed is responsible for delivering the data that is needed to run and test strategies. There are no restrictions on the type of data that a `Feed` can emit. It can be as simple as stock prices or as complex as satellite images. A `Feed` is used to deliver historic and live data.
...
=== Time
The time attribute in an event is of the type `java.time.Instant`. So they can be easily processed and compared without having to worry about timezones. When a feed delivers events, they are always in chronological order.
...
The following table shows which out-of-the-box feeds support which types of price-actions:

[cols="<,<,<,<"]
|===
|Feed |Type| Historic prices|Live prices
|Generic CSV| File| PriceBar, PriceQuote| -
...
You can also convert other feeds into an AvroFeed that you can later use for back-testing. This works with both historic data feeds and live data feeds. The current limitation is that it only works with price actions and not other type of actions like image data or social media posts.
