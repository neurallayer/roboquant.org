...
A feed wraps its information in an `Event` and puts that `Event` on an `EventChannel`. This is all done asynchronously from the rest of the trading logic. And although an `EventChannel` is used to deliver the events, this channel is normally only accessed by internal _roboquant_ logic. A xref:strategy.adoc[strategy] will be passed just the `Event` object.
