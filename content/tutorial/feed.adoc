...
=== Memory Constraints
The default `CSVFeed` loads all the data found in all the files into memory during initialization. The benefit is that after initialization the CSVFeed is very fast when used in back tests. And since you reuse the feed in concurrent back-tests, this becomes even more important when doing extensive back testing.

However, a possible downside is memory usage when working in a memory-constrained environment. So there is also a CSV Feed implementation that only reads the data when required. This is substantial slower, but consumes a lot less memory. It supports the same configuration, the only difference is that you need to use `LazyCSVFeed` instead of the regular `CSVFeed`
...
Avro files make it easy to store large amount of data in a single file and efficiently use it during back testing. The AvroFeed implementation of _roboquant_ only loads data in memory when it is required, so very large datasets can be used without having to worry about memory issues. Also since the Avro Feed uses a single file, it is easier to distribute it than for example directories full of CSV files.


As you might already guessed, but the use of an `AvroFeed` comes highly recommended.
