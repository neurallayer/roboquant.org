...
== Simple Signals
When instantiating a signal, only the Asset and Rating are required parameters:
...
It is up to the xref:policy.adoc[policy] if/how these attributes are used when generating the orders. But for example, the probability could be used to determine the sizing of the order, a higher probability implying a larger size.
