...
. Re-balancing your portfolio at regular intervals (like monthly) based on some risk profile. Since a `Strategy` doesn't have access to an `Account` and therefore neither to the positions in the `Account`, this needs to be done in a `Policy`.
