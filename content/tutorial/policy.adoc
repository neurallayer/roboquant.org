...
== What is a policy
The `Policy` component is responsible for creating the orders that will be placed at the broker. The most common use-case is that a `Policy` does this based on the signals it receives from a `Strategy` (but there are other use-cases).
