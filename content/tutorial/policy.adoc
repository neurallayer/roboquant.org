...
=== FlexPolicy
This is the default policy that will be used if no other policy is specified when creating an instance of the `Roboquant` class. It provides several configuration properties that influence its behavior.

You can also extend the FlexPolicy and overwrite some of its methods. For example, to create your own order types you would implement something like this:
...
== Advanced Policies
Most commonly, a `Policy` is used to transform signals it receives from the `Strategy` into orders. But there are other use cases where the policy is not using Signals to create orders. Common use cases are:
...
. Advanced machine learning techniques like reinforcement learning that train the algorithm to directly create orders based on events (without creaing intermediate signals).
